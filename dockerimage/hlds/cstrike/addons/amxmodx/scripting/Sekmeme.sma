...
new g_Cvar_SekmemeAktif, g_Cvar_SekmemeAll, g_Cvar_SekmemeTakim, g_Cvar_SekmemeAdmin;
...
public plugin_init() { 
	...
	g_Cvar_SekmemeAktif = register_cvar("sekmeme_aktif", "1");
	g_Cvar_SekmemeAll = register_cvar("sekmeme_all", "0");
	g_Cvar_SekmemeTakim = register_cvar("sekmeme_takim", "2");
	g_Cvar_SekmemeAdmin = register_cvar("sekmeme_admin", "1");
...
{
	if(g_fix_punchangle[id] && get_pcvar_num(g_Cvar_SekmemeAktif) == 1)
	{
		if(get_pcvar_num(g_Cvar_SekmemeAdmin) == 1 && get_user_flags(id) & ADMIN_YETKI)
		{
			g_fix_punchangle[id] = false 
			set_pev(id, pev_punchangle, Float:{0.0, 0.0, 0.0}) 
			return FMRES_HANDLED 
		}
		else if(get_pcvar_num(g_Cvar_SekmemeAll) == 1)
		{
			g_fix_punchangle[id] = false 
			set_pev(id, pev_punchangle, Float:{0.0, 0.0, 0.0}) 
			return FMRES_HANDLED 
		}
		else if(get_user_team(id) == get_pcvar_num(g_Cvar_SekmemeTakim))
		{
			g_fix_punchangle[id] = false 
			set_pev(id, pev_punchangle, Float:{0.0, 0.0, 0.0}) 
			return FMRES_HANDLED 
